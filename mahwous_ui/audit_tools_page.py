# -*- coding: utf-8 -*-
"""
صفحة «التدقيق والتحسين»: المقارنة الذكية، مدقق المتجر، معالج SEO.
تعتمد على `audit_tools_core` فقط ولا تعدّل محرك v26 الرئيسي.
"""
...
# ══════════════════════════════════════════════════════════════════
#  مسارات البيانات — جذر تطبيق v26 (مجلد الأب لـ mahwous_ui)
# ══════════════════════════════════════════════════════════════════
...
def configure_app_logging() -> logging.Logger:
    log = logging.getLogger("mahwous.audit_tools")
    if log.handlers:
        return log
    ...
    try:
        ...
        fh = logging.FileHandler(
            os.path.join(DATA_DIR, "logs", "mahwous_audit_tools.log"),
            encoding="utf-8",
        )
                ...
                if "audit_fixed_df" in st.session_state and st.session_state.audit_fixed_df is not None:
                    ...
                    with aud_e1:
                        st.download_button(
                            ...
                            "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                            use_container_width=True, key="sa_dl_audit_fix_x")
                    with aud_e2:
                        st.download_button(
                            ...
                            "text/csv", use_container_width=True, key="sa_dl_audit_fix_c")
...
def render_audit_tools_page():
    """تبويبات المقارنة الذكية، مدقق المتجر، ومعالج SEO."""
    _audit_tools_inject_css()
    _audit_tools_init_state()

    c1, c2 = st.columns([4, 1])
    with c1:
        st.title("🛠️ التدقيق والتحسين")
    with c2:
        if st.button("← العودة للتطبيق", key="audit_tools_back_main", use_container_width=True):
            st.session_state.audit_tools_mode = False
            st.rerun()

    st.caption("تتطلب مفتاح Anthropic في الإعدادات أو Secrets لاستخدام توليد المحتوى بالذكاء الاصطناعي.")
